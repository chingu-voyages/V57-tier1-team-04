        ...
        <div className="flex items-center gap-2 pr-8">
                <div className="flex items-center gap-3">
                    ...
                    {pr.comments && pr.comments.length > 0 && (
                        <div className="pr-comments flex items-center gap-1 text-gray-600 text-xs md:text-base"
                             aria-label={`${pr.comments.length} comment${pr.comments.length !== 1 ? 's' : ''}`}
                            >
                            <span className="flex items-center justify-center gap-1 text-xs md:text-base font-medium">
          ...
          <span className="text-green-600 font-semibold text-xs md:text-base">#{pr.number}</span>
        </div>

        {/* Icon shows the status: open or close the card */}

        <span className="text-xs md:text-base font-bold text-gray-500 transform transition-transform duration-300 ml-4"
          aria-hidden="true"
        >
          {isOpen ? '▲' : '▼'}
        </span>
      ...
      >
        ...
        <div className="overflow-hidden">
          <div className="mt-2 pt-2 border-t border-gray-200">
            ...
            {/* Description */}
            <p className="text-gray-500 text-xs md:text-base mb-3">{pr.body || "No description provided"}</p>

            {/* Details */}
            <div className="semi-card grid grid-cols-1 md:grid-cols-2 gap-3 text-xs md:text-base bg-gray-200 p-3 rounded-lg">
                <div>
